...
/**
 * REST controller for managing {@link com.neoproc.autoplanilla.domain.Company}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Company> partialUpdateCompany(
        ...
        Optional<Company> result = companyRepository
            ...
            .map(existingCompany -> {
                ...
                if (company.getTipoIdentificacion() != null) {
                    existingCompany.setTipoIdentificacion(company.getTipoIdentificacion());
                }
                if (company.getIdentificacion() != null) {
                    existingCompany.setIdentificacion(company.getIdentificacion());
                }
                if (company.getCorreoElectronico() != null) {
                    existingCompany.setCorreoElectronico(company.getCorreoElectronico());
                }
                if (company.getNombreComercial() != null) {
                    existingCompany.setNombreComercial(company.getNombreComercial());
                }
                if (company.getTelefono() != null) {
                    existingCompany.setTelefono(company.getTelefono());
                }
                if (company.getProvincia() != null) {
                    existingCompany.setProvincia(company.getProvincia());
                }
                if (company.getCanton() != null) {
                    existingCompany.setCanton(company.getCanton());
                }
                if (company.getDistrito() != null) {
                    existingCompany.setDistrito(company.getDistrito());
                }
                if (company.getBarrio() != null) {
                    existingCompany.setBarrio(company.getBarrio());
                }
                if (company.getSennas() != null) {
                    existingCompany.setSennas(company.getSennas());
                }
                if (company.getEstado() != null) {
                    existingCompany.setEstado(company.getEstado());
                }
